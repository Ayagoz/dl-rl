...
class BasicTranslationModel:
    def __init__(self, name, inp_voc, out_voc,
        ...
        with tf.variable_scope(name):
            ...
            self.enc = tf.nn.rnn_cell.MultiRNNCell([tf.nn.rnn_cell.LSTMCell(hid_size) for _ in range(2)])

            self.dec_start = [L.Dense(hid_size) for _ in range(4)]

            self.dec = tf.nn.rnn_cell.MultiRNNCell([tf.nn.rnn_cell.LSTMCell(hid_size) for _ in range(2)])
