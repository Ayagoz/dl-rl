...
class UpTranslationModel:
    def __init__(self, name, inp_voc, out_voc,
        ...
        with tf.variable_scope(name):
            ...
            self.enc = tf.nn.rnn_cell.MultiRNNCell([tf.nn.rnn_cell.LSTMCell(hid_size) for i in range(2)])
            
            self.dec_start = [L.Dense(hid_size) for i in range(4)]
            
            self.dec = tf.nn.rnn_cell.MultiRNNCell([tf.nn.rnn_cell.LSTMCell(hid_size) for i in range(2)])
    ...
    def encode(self, inp,  **flags):
        ...
        enc_last = [enc_last[i][j] for i in range(2) for j in range(2)]

        dec_start = [layer(hid_state) for hid_state, layer in zip(enc_last, self.dec_start)]
        
        dec_start = [tf.nn.rnn_cell.LSTMStateTuple(dec_start[i], dec_start[i+1]) for i in [0,2]]
    ...
    def decode(self, prev_state, prev_tokens, **flags):
        ...
        #[prev_dec, prev_dec2] = prev_state

        [prev_dec] = prev_state

        prev_emb = self.emb_out(prev_tokens[:,None])[:,0]

        new_dec_out,new_dec_state = self.dec.call(prev_emb, prev_dec)
    ...
    def symbolic_translate(self, inp, greedy=False, max_len = None, eps = 1e-30, **flags):
        ...
        def step(blob,t):
            ...
            step_result = [list(h_new[0])] + [logits, tf.cast(y_new,y_prev.dtype)]
            return step_result
        ...
        # states_seq = [tf.concat((init[None], states), axis=0)
        #               for init, states in zip(first_state[0], states_seq)]
